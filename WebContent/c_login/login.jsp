...
<<<<<<< HEAD:WebContent/login/login.jsp
...
>>>>>>> 9cffe6be1feca6357727c1c4afa162d2250a0032:WebContent/c_login/login.jsp
<title>로그인</title>
...
<body onload='document.MemberLoginFm.fm_id.focus();'>
	...
	<div id="conainer">
		<div id="nav">
			<div class="nav_txt">
				<p>
					<a href="/index.php">Home</a> <span class="padd">&gt;</span> <span>로그인</span>
				</p>
			</div>
		</div>
		<!-- //nav : e -->
			...
			<div class="login_cont">
				...
				<div class="log_box">
					<div class="log_lef">
						...
						<form method="post" name="MemberLoginFm"
							action="/00_include/global/login.php" onSubmit="return false;">
							<input type="hidden" name="fm_url" value=""> <input
								type="hidden" name="fm_str" value=""> <label for="fm_id"
								class="tx_skip">아이디 입력</label>
							<div>
								<input type="text" id="fm_id" class="log_inp01" name="fm_id" />
							</div>
							<div>
								<input type="password" id="fm_passwd" class="log_inp02"
									name="fm_passwd" onKeyPress="javascript:LoginEnterCheck();" />
							</div>
							<label for="fm_passwd" class="tx_skip">비밀번호 입력</label>
							<div class="login_check">
								<p class="fl">
									<input type="checkbox" id="saveID" name="saveID" value="Y"
										checked="checked" /> <label for="saveID">로그인 상태유지</label>
								</p>
								<a href="javascript:FindID();" class="pass_find">아이디 패스워드 찾기
									&gt;</a>
							</div>
							<p>
								<a href="javascript:LoginCheckIt();" class="btn_login">로그인</a> <a
									href="/08_MELC/member/member_join.php" class="btn_join">회원가입</a>
							</p>
						</form>
					</div>
